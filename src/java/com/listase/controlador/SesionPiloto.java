...
public class SesionPiloto implements Serializable {
    ...
    public void setMoverPiloto(short moverPiloto) {
        this.moverPiloto = moverPiloto;
    }
    ...
    public void adelantarPosicion() throws PilotoExcepcion1 {
        try {
            int posi = listaPiloto.obtenerPosicion(pilotoSeleccionado) - moverPiloto;
            short tama = listaPiloto.contarNodos();
            if (posi > tama || posi < 0) {
                JsfUtil.addErrorMessage("posicion " + posi + "Invalida");
            } else {
                Piloto temp = listaPiloto.obtenerPiloto(pilotoSeleccionado);
                listaPiloto.caidaPiloto(pilotoSeleccionado);
                listaPiloto.adelantarPiloto(temp, (short) posi);
                pintarLista();
